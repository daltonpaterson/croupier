...
st.set_page_config(
    page_title="Roulette Finale Neighbour Bets"
)
...
if not st.session_state.difficulty_chosen:
    st.markdown("<h1 style='text-align: center;'>Finale Neighbour Bets</h1>", unsafe_allow_html=True)
    ...
    def generate_question():
        finale = random.choice(range(0, 10))  # 0 to 9
        amount = random.choice(question_pool)

        # Decide multiplier
        multiplier = 4 if finale <= 6 else 3

        # Compute answer
        answer = ((amount * 5) * multiplier)  # because "amount to multiply" is amount * 25

        # Question text
        question = f"Finale {finale} neighbours by {amount}"
    ...
    if return_to_menu.button("Return to Menu", use_container_width= True):
        st.session_state.clear()
        st.switch_page("pages/roulette_finale_neighbour_bets.py")
